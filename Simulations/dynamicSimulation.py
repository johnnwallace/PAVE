...
class Boat:
    def __init__(self, x, y, theta, speed, angularSpeed, motor):
        ...
        self.DRAG_COEFFICIENT = (
            0.7  # Completely made up but the motor thrust is also made up to counter it
        )

        motor.setThrust(
            self.calculateDrag(self.WIDTH * 0.3)
        )  # Temporary measure to set a constant thrust to maintain the initial cruising speed
    ...
    def calculateForwardAcceleration(self):
        ...
        drag = self.calculateDrag(self.WIDTH * 0.3)
    ...
    def calculateRotationalAcceleration(self):
        ...
        dragMoment = 2 * (self.calculateDrag(self.LENGTH * 0.3) * self.LENGTH / 2)
...
controller = PID(setpoint, kp, ki, kd, ks, maxMotorAngle)
